...
class ObjLoader:
    ...
    @staticmethod
    def load_model2(path: str) -> (np.ndarray, np.ndarray):
        vert_coords = []
        tex_coords = []
        norm_coords = []
        indices = []
        ...
        with open(path, 'r') as file:
            for line in file.readlines():
                values = line.split()
                if len(values) == 0 or values[0] == '#' or values[0] == 'f':
                    continue
                if values[0] == 'v':
                    vert_coords.append([float(i) for i in line.split()[1:]])
                elif values[0] == 'vt':
                    tex_coords.append([float(i) for i in line.split()[1:]])
                elif values[0] == 'vn':
                    norm_coords.append([float(i) for i in line.split()[1:]])
                elif values[0] == 'f':
                    vertexes = line.split()[1:]
                    for i in vertexes:
                        vertex_info = i.split('/')
                        vertex_idx = int(vertex_info[0]) - 1
                        tex_idx = int(vertex_info[1]) - 1
                        norm_idx = int(vertex_info[2]) - 1
                        indices.append(vertex_idx)
                        buffer.append(vert_coords[vertex_idx])
                        # buffer.append(tex_coords[tex_idx][:2])
        ...
        return np.array(indices, dtype=np.uint32), \
               np.concatenate(vert_coords, dtype=np.float32)
